       ...
       <div id="hotel_errmsg"> </div> 
          ...
          <button type="submit" class="btn-search" onclick="return validation_all();">Search</button>
			...
			<?php $count= 1; foreach($foodfiesta_offers_list as $food_offers) {  
                                    $name = base64_encode($food_offers->me_name);
			...
			<td>
              <?php if($food_offers->image == ''){
               ...
               ?>
                 <?php $img='<img width="" src="'.Theme::asset()->url('foodfiesta_images/'.$segment_image.'.jpg').'">'; echo HTML::decode(HTML::link('get_hotels/'.$name.'/'.$city.'/'.$seg.'', $img)); ?>
               <?php } else { ?>
               
                <?php $img='<img width="" src="uploads/foodfiesta_images/'.$food_offers->image.'">'; 
echo HTML::decode(HTML::link('get_hotels/'.$name.'/'.$city.'/'.$seg.'', $img)); ?>
                <?php } ?>
		<?php  if(strlen($food_offers->offer_details) >30){
               $offer_name = preg_replace('/\s+?(\S+)?$/', '', substr($food_offers->offer_details,0,25)).'...';
                }else{
