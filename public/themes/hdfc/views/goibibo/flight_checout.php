...
<span class="newspan size12" style="margin-left: 8px;">Gender</span>
				...
				<input type="radio"  name="gender<?php echo $i;?>" id="gender<?php echo $i;?>" value="2"  >Female
						...
						<input type="hidden" name="local_inter" value= "<?php echo $type; ?>" />
...
//                                            }
					...
					<div class="padding20">					
						...
						<span  class="right" style="text-align: right;" >₹ 
                                       ...
                                       <div class="col-md-9">
					<span class="size12">Coupon Code</span>
					<input type="text" name="search_code" id="coupon" class="form-control " placeholder="">
					<span class="error_cp" style="color:red"></span>
					<input class="btn-search" type="button" id="coupon_code" value="Apply Coupon Code">
					</div>
                                      ...
                                      //  $offer = Offerhelper::offer_calculate(7,$amount);
                                         ?> <input type="hidden" id="partner_id" name="partner_id" value="7"></input>
                                      <?php  }
                                        else
                                        {
                                      //  $offer = Offerhelper::offer_calculate(5,$amount); ?>
					<input type="hidden" id="partner_id" name="partner_id" value="5"></input>   
                                      <?php   } ?>
					...
					<div class="offer_show">				
                                        ...
                                        <span class="right " style="color:#e20613;" > ₹ <div class="discount_offer" style="display: inline;"></div> </span>
					<input type="hidden" id="dis_amount" name="discount_amount" value=""/>
					</div>
                                        </div>
...
<span class="right" style="text-align: right;">₹ <div class="discountted_amount" style="display: inline;"><?php echo $amount ;?></div> </span>
	
						<input type="hidden" id="discountted_amount" name="discountted_amount" value=""/>
						<div class="clearfix"></div>
					</div>
 ...
 <input type="hidden" id="coupon_codes" name="coupon_code" />
...
$("#coupon_code").click(function(){
	var html_content = '<div class="topopup-inner"><div class="close"></div><span class="ecs_tooltip">Press Esc to close <span class="arrow"></span></span><div id="popup_content" ></div></div>'; 
	$("#toPopup").html(html_content);
  	var partner_id  =  $("#partner_id").val();
	var amount  =  "<?php echo $amount; ?>";
	var coupon  =  $("#coupon").val();
	var type  =  '<?php echo $type; ?>';	
	if(coupon == ''){
		$(".error_cp").html('Please Enter Coupon Code');
	}
	else{
	var _token ='<?php echo csrf_token(); ?>';
  $.ajax({
                      type: "POST",
                      url: "<?=URL::to('verify_coupon')?>",
                      data: {
               			"partner_id":partner_id,"_token":_token,"amount":amount,"coupon":coupon,"type":type
              		    },             
			...
			if(obj.status == 'success'){
				$(".error_cp").html('');
				$(".offer_show").show();
				$(".discount_offer").html(obj.discount);
				$("#dis_amount").val(obj.discount);
				var dis_amt = amount - parseInt(obj.discount);
				$(".discountted_amount").html(dis_amt);
				$("#discountted_amount").val(dis_amt);
				$("#coupon_codes").val(coupon); 
			}else{
					$(".error_cp").html('Coupon code Not Valid');
			 }
