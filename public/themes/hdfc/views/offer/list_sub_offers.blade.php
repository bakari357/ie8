...
<?php    $url= explode('/',Request::url());  if(isset($url[7])) { $card_selected=$url[7];} else { $card_selected =''; }  
?> 		
<form method="post" action="<?php print_r(Config::get('app.url')); echo 'list_offer/'.$url[6].'/'.$url[7];?>" id="hotel_checkout" name="hotel_checkout" >	
...
<?php echo Form::select('card',$card_type,$card_selected,array( "class"=>"form-control segment","id"=>"card" ,"name"=>"card","style"=>"width: 189px;margin-left: -24px;")); ?>
			</div>
		</div>				
...
echo '<font color="red" style="margin-left:409px;" > <b>No offers are available currently. </b></font>';
        ...
        $offerurl= explode('/',Request::url());  if(isset($offerurl[6])) {  $off_types=$offerurl[6]; 
             ...
             <?php if($off_types==15) { 
                  ...
                  $img='<img width="232" hight="118" style="height: 61px;width: 233px;border: 1px solid #EAEBEC;" src="'.$image.'">'; echo HTML::decode(HTML::link('get_offers/'.$off_types.'/'.$name.'/'.$city.'/'.$card_selected.'', $img)); } else {
                ...
                if($off_types==15){
                   $img='<img width="226" hight="61" style="height: 61px;width: 233px;border: 1px solid #EAEBEC;" src="'.Theme::asset()->url('offer_images/'.$daily_offer->image).'">'; echo HTML::decode(HTML::link('get_offers/'.$off_types.'/'.$name.'/'.$city.'/'.$card_selected.'', $img)); 
               
            
                
                }  else { 
                
                 $img='<img width="226" hight="61" style="height: 61px;width: 233px;border: 1px solid #EAEBEC;" src="'.Theme::asset()->url('offer_images/'.$daily_offer->image).'">'; echo HTML::decode(HTML::link('offer_details/'.$daily_offer->id.'/'.$off_types.'', $img));
                 } } ?>
              
		<?php if(strlen($daily_offer->short_description) >30){
	...
	<br/>				
<div class="cpadding1"> <font color="#034387" style="font-size:13px;"><b><?php echo $daily_offer->deal_name; ?></b></font><br/>  <span style="font-size:12px;"> <?php echo $description; ?></span></div>
