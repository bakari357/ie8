...
$('#resultss').html('<div id="result" align="center"><div style="padding-top:120px;"><section class="retrive_password"><h3>No Results Found</h3><div class="retrive_password_content"><img style="padding-right: 57px;"</div></section></div></div> </div>');
	...
	//$('#total_movies').text(data.length+' Hotels available');

	var offset = this.data.length;

	//if (offset == 199) this.clearStreamingTimer();
	...
	var percent = Math.round(((this.data.length)/this.services.length)*100);
	...
	percent=100;
      ...
      if (percent == 100) $('#stream_progress').fadeOut(1000);
	var cnt_fly=data.length;
	if(cnt_fly==0)
	{
...
$('#result').html('');
   ...
   //   console.log(services);
        return FilterJS(data,services, "#movies", view, options);
