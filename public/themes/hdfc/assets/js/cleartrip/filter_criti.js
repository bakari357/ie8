...
(function(window) {
  ...
  'use strict';
var name='';
  var FilterJS = function(data, container, view, container1, view1, options) {
    new _FilterJS(data, container, view, options);
    return new _FilterJS1(data, container1, view1, options);
  };
  ...
  $.fn.filterjs = function(data, view, options) {
   ...
   if ($this.data('fjs1')) return;
    $this.data('fjs1', new _FilterJS(data, $this, view, options));
  ...
  var _FilterJS1 = function(data, container, view, options) {
    var property_count = 0;
    ...
    this.options.callbacks = this.options.callbacks || {};
    this.execCallBack('after_init', this.record_ids);
    this.execCallBack('after_add1', this.data);
    this.options.filter_types = this.options.filter_types || {};
  ...
  _FilterJS1.prototype = {
    ...
    render: function(data, offset) {
      ...
      for (var i = 0, l = data.length; i < l; i++){
        record = this.getRecord(i, data);
        el = $(this.view(record));
        el.attr({id: this.root + '_' + record[this.id_field], 'data-fjs1': true});
        el = $container.append(el);
      }
    },
    ...
    addData: function(data){
      ...
      this.render(data);
      this.buildCategoryMap(data);
      this.execCallBack('after_add1', data)
      this.filter();
    },
