function get_checked_cleartrip()
{
...
get_checked_goibibo_onward(combined_price);

}



function get_checked_goibibo_onward(goibibo)
{
...
//    alert(onward_airline);
    ...
    type: "POST",
    data : {req:req,fcode:onward_fcode,_token:token},
    async:true,
    dataType: 'json',
    success: function(response)
    {
//	        if(response.trim() =='Flights not available.'|| response.trim()== 'Invalid carrier code:')
//		{
//		onward_total=0;
//		}
//		else
...
//		}
                get_checked_goibibo_return(goibibo,onward_total);
...
function get_checked_goibibo_return(goibibo,onward_total)
{
    var onward_airline= $('input[name=onward]:checked').closest('td').siblings('td.select_airlines').text();
    var onward_fcode=$('input[name=onward]:checked').closest('td').siblings('td.select_fcode').text();
    var return_airline=$('input[name=return]:checked').closest('td').siblings('td.select_airlines').text();
...
//     alert(return_airline);
    ...
    url : "price_search_cleartrip_round",
    type: "POST",
    data : {req:req,fcode:return_fcode,_token:token},
    async:true,
    dataType: 'json',
    success: function(response)
    {  
...
function fare_render(goibibo,onward_total,return_total,clear_total)
{
     ...
     $('#proceed_go').hide();
      $('#proceed_clear').show();
    $(".fare_pad1").html('<img src="themes/hdfc/assets/images/cleartrip-logo1.png" height="21px" style="padding-left: 20px;" >  &#8377 '+goibibo);
    if(onward_total!=0 && return_total!=0 && typeof clear_total!="undefined")
    {
    $(".cleartrip_pad").html('<img src="themes/hdfc/assets/images/goibibo.png" height="17px" style="padding-left: 20px;" > &#8377 '+clear_total);
...
$.ajax({
    ...
    success: function(response)
    {
		...
		$(".fare_loader").hide();
		$(".cleartrip_pad").html('<img src="themes/hdfc/assets/images/cleartrip-logo1.png" height="21px">  &#x20b9 '+combined_price);
		$("#proceed").show();
                $("#proceed_go").hide();
		$(".cleartrip_pad").hide();
		...
		if(clear_total != "" && typeof clear_total!="undefined")
		{
		$(".fare_pad1").html('<img src="themes/hdfc/assets/images/goibibo.png" height="17px"> &#x20b9 '+clear_total);
                $("#proceed_go").show();
                }
		var fares = [combined_price,clear_total];
		var lowest=fares.sort(function(a, b){return a-b});
