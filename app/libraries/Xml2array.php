<?php
class Xml2array {
    public static function xml2array_test($contents, $get_attributes=1, $priority = 'tag') { 
	...
	public static function array_to_csv($query,$heading, $delim = ",", $newline = "\n", $enclosure = '"')
	{
...
// 		}
		...
		foreach ($heading as $name)
		{
			...
			foreach($name as $val)
			{
			$out .= $enclosure.str_replace($enclosure, $enclosure.$enclosure, $val).$enclosure.$delim;
			}
		}
	...
	//public static function array_to_xml($array, $level=1) {
                ...
                // recrusive call.
                Xml2Array::array_to_xml($value, $rootNodeName, $node);
                }
                else
                {
                // add single node.
