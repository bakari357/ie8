<?php
use Carbon\Carbon;
class Goibibo extends BaseController  {
/*
	|--------------------------------------------------------------------------
	...
	*/
		...
		$flight['bread_crumb']=array('Home'=>'/');
        	...
        	$this->load_reward_region('hungama','home',1);
       		return $this->render_reward_theme('goibibo.home',$flight); 
         }
	...
	public function indexint()
	{
        	...
        	$this->load_reward_region('hungama','home',1);
       		return $this->render_reward_theme('goibibo.inthome',$data); 
	}
...
$data['c_ratio']=0.70;
				...
				return $this->render_reward_theme_hotels('goibibo.'.$view,$data);
			}
...
public function price_search()
	...
	public function price_search_round()
	{
		...
		//$url='https://api.cleartrip.com/air/1.0/search?from='.$source.'&to='.$destination.'&depart-date='.$odate.'&return-date='.$rdate.'&adults='.$adult.'&children='.$child.'&infants='.$infant.'&carrier='.$car_id.'&cabin-type='.$param[6].'&sort=asc';
	...
	{ 
            ...
            $clear_select=$data['clear_select']=json_decode($_POST['clear_select']);
            ...
            $return_select=json_decode($_POST['return_select']);
            ...
            if(isset($_POST['return_select']) && $_POST['return_select']<>'')
            {
            $return_select=$data['return_select']=json_decode($_POST['return_select']);
            }
...
//                 echo "<pre>";print_r($passsenger_details_return);
        ...
        {    
            ...
            $_POST=$data;
              //Offer Coupon Code recheck  
		
			 $offer = Offerhelper::offer_calculate_coupon($_POST['partner_id'],$_POST['total_amount'],$_POST['coupon_code'],$_POST['local_inter']); 
			$data['offer'] = $offer;
			if(!empty($offer)){
			
				$data['discount'] = $offer['discount_amount'];
				$data['discountted_amount'] = (int)$_POST['total_amount'] -  (int)$data['discount'];  
			}
			else{
				$data['discount'] = 0;
              ...
              //Offer Coupon Code recheck Ends here                   
