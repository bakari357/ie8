<?php 
class Walletsrecharge extends AdminBaseController {
protected $wallets;

   public function index()
     {
	 $walletsrecharge = Walletrecharge::paginate( Config::get('15'));
       // $wallets=Wallet::where('role_id', '=', '3')->where('sub_user','<>',1)->get();
	return $this->load_admin_theme('walletsrecharge','index',array('walletsrecharge'=>$walletsrecharge));      
     }
	...
	$cities  = DB::table('cities')->where('country_code', 'IN')->get();
        $images = array();
	return $this->load_admin_theme('walletsrecharge','create',array('images'=>$images,'states'=>$states,'cities'=>$cities)); 
    }
    ...
    {  
	 ...
	 $validator = Validator::make(Input::all(), $rules);
          	if ($validator->fails())
		  {
			...
			$wallet_id = DB::table('wallets')->insertGetId(array(
			...
			'over_draft_limit'       =>   Input::get('service'), 
                        'over_draft_duration'    =>  Input::get('service'), 
			'discount_type'          =>   Input::get('discount'), 
                        'discount_regular_basis' =>   Input::get('discount_regular'), 
                    ...
                    $wal_id = DB::table('wallet_extended')->insertGetId(array(
...
/* if($_POST['password']<>''){
    ...
    {   
	$rechargelist = DB::table('wallet_recharge')->where('recharge_mode', '=', '2')->orWhere('recharge_mode', '=', 3)->get();
        return $this->load_admin_theme('wallet_recharge','index',array('rechargelist'=>$rechargelist));       
    }

	public function show_rechargelist($id)
    {   
	$rechargelist = DB::table('wallet_recharge')->where('id', '=', $id)->get();
        return $this->load_admin_theme('wallet_recharge','show',array('rechargelist'=>$rechargelist[0]));       
    }
}
