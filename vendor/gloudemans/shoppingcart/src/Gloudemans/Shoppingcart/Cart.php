...
class Cart {
	...
	public function __construct($session, $event)
	{
		...
		$this->instance = 'main';
		//Here added cart needed details by udayakumar
		...
		$this->_cart_contents['cart_subtotal_points']		= 0;
		...
		$this->_cart_contents['cart_total_points'] 		= 0;
		$this->_cart_contents['total_items'] 			= 0;
		$this->_cart_contents['shipping_total'] 		= 0;
		// tax
		$this->_cart_contents['tax'] 				= 0;
		$this->_cart_contents['offer_discount'] 		= 0;
		$this->_cart_contents['taxable_offer_discount']	= 0;
		...
		//its is check and use
                $this->_cart_contents['shipping']['method']= true;
		...
		{    
			...
			{     
				...
				{    
					...
					$this->addRow($item['id'], $item['name'], $item['qty'], $item['price'],$details,$options);
				}
			...
			return $this->addRow($id['id'], $id['name'], $id['quantity'], $id['price'],$details,$options);
		}

		// Fire the cart.add event
		$this->event->fire('cart.add', compact('id', 'name', 'quantity', 'price', 'details','options'));
                    
		return $this->addRow($id, $name, $qty, $price,$details, $options);
	}
	...
	public function gift_card_discount()
	{
	...
	{
		return $this->_cart_contents['offer_discount'];
	}
	
	public function dealTotal($amount)
	{

	        $this->_cart_contents['deal_total']=$amount;
	}
	
	public function deal()
	{
	if(isset($this->_cart_contents['deal_total']))
	return round($this->_cart_contents['deal_total'], 2);
